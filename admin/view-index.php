<?php
/**
 * 	Madison Admin Page
 * 
 * 	@copyright Copyright &copy; 2012 by The OpenGov Foundation
 *	@license http://www.gnu.org/licenses/ GNU GPL v.3
 */
?>

<a style="display:block;margin-bottom:10px;" href="/">Home</a>
<h1>Administrative Index</h1>
<ul>
	<!--<li><a href="/admin/info">Site Info</a></li>-->
	<li><a href="/admin/nav">Edit Navigation Bar</a></li>
	<li><a href="/admin/company-approval">Company Approvals</a></li>
	<!--<li><a href="/admin/users">Moderate Users</a></li>
	<li><a href="/admin/notes">Moderate Notes</a></li>
	<li><a href="/admin/import">Import Documents</a></li>-->
	<li><a href="/admin/docs">View/Edit Documents</a></li>
</ul>
